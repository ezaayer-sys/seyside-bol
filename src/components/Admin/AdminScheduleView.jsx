...
function NewLoadModal({ customers, carriers, defaultDate, onClose, onSave, userId }) {
  ...
  const inputStyle = { width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', padding: '9px 12px', color: '#f1f5f9', fontSize: '13px', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
  const labelStyle = { display: 'block', color: '#94a3b8', fontSize: '11px', fontWeight: '600', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.05em' };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: '#1e293b', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ color: '#f1f5f9', fontSize: '18px', fontWeight: '700', margin: 0 }}>Add New Load</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            ...
            {specs.length > 0 ? (
              <div style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '8px', padding: '12px' }}>
                <div style={{ color: '#34d399', fontSize: '12px', fontWeight: '600', marginBottom: '8px' }}>✓ Auto-filled from customer profile</div>
                {specs.map((s, i) => (
              ...
              <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '8px', padding: '12px' }}>
                <div style={{ color: '#f59e0b', fontSize: '12px' }}>
                  {customerId ? '⚠ No specs on file. Add specs to customer profile first.' : 'Select a customer to auto-fill specs.'}
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any special instructions..." rows={2} style={{ ...inputStyle, resize: 'vertical' }} />
          </div>
        </div>

        {customerId && barrelCount && shipDate && (
          <div style={{ marginTop: '16px', background: specs.length > 0 ? 'rgba(16,185,129,0.06)' : 'rgba(245,158,11,0.06)', border: `1px solid ${specs.length > 0 ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}`, borderRadius: '8px', padding: '10px 14px', color: specs.length > 0 ? '#34d399' : '#f59e0b', fontSize: '12px', fontWeight: '600' }}>
            {specs.length > 0 ? '✓ Load is complete — supervisor can process this order' : '⚠ Load is incomplete — supervisor cannot process until specs are added'}
          </div>
        )}
...
function LoadCard({ load, onClick, compact }) {
  const color = load.customer?.category ? CATEGORY_COLORS[load.customer.category] || '#6b7280' : '#6b7280';
  return (
    <div onClick={() => onClick(load)} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: `3px solid ${color}`, borderRadius: '6px', padding: compact ? '6px 8px' : '10px 12px', cursor: 'pointer', marginBottom: '4px', transition: 'background 0.15s' }}
      onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'}
      onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.03)'}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '6px' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ color: '#e2e8f0', fontSize: compact ? '11px' : '13px', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{load.customer?.name || '—'}</div>
          <div style={{ color: '#64748b', fontSize: compact ? '10px' : '11px', marginTop: '2px' }}>{load.barrel_count} bbls {!compact && load.bol_number ? `• ${load.bol_number}` : ''}</div>
        </div>
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', flexShrink: 0 }}>
          {!compact && <StatusBadge status={load.status} />}
          {!isLoadReady(load) && <span style={{ color: '#f59e0b', fontSize: '12px' }} title="Incomplete">⚠</span>}
        </div>
      </div>
    </div>
  );
...
function LoadDetailPanel({ load, onClose, onRefresh, userId }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    await supabase.from('loads').update({ ship_date: shipDate, po_number: poNumber || null, barrel_count: parseInt(barrelCount), notes: notes || null, status, updated_by_id: userId, updated_at: new Date().toISOString() }).eq('id', load.id);
    setSaving(false);
    setEditing(false);
    onRefresh();
  };
  ...
  const inputStyle = { width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', padding: '8px 12px', color: '#f1f5f9', fontSize: '13px', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
  const ready = isLoadReady(load);

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: '#1e293b', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            <h2 style={{ color: '#f1f5f9', fontSize: '18px', fontWeight: '700', margin: '0 0 4px' }}>{load.customer?.name || '—'}</h2>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              {load.bol_number && <span style={{ color: '#c4a35a', fontSize: '13px', fontWeight: '600' }}>{load.bol_number}</span>}
        ...
        <div style={{ background: ready ? 'rgba(16,185,129,0.06)' : 'rgba(245,158,11,0.06)', border: `1px solid ${ready ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}`, borderRadius: '8px', padding: '10px 14px', marginBottom: '20px', color: ready ? '#34d399' : '#f59e0b', fontSize: '12px', fontWeight: '600' }}>
          {ready ? '✓ Complete — supervisor can process this load' : '⚠ Incomplete — missing specs. Supervisor cannot process.'}
        </div>
        ...
        {editing ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ flex: 2, padding: '10px', background: '#c4a35a', border: 'none', borderRadius: '8px', color: '#1a1a1a', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>{saving ? 'Saving...' : 'Save Changes'}</button>
            </div>
          </div>
        ) : (
          <>
            ...
            {load.barrel_specs_custom && load.barrel_specs_custom.length > 0 && (
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '12px', marginBottom: '20px' }}>
                <div style={{ color: '#94a3b8', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', marginBottom: '8px' }}>Barrel Specs</div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={handleDelete} style={{ flex: 1, padding: '10px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '8px', color: '#ef4444', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button>
              <button onClick={() => setEditing(true)} style={{ flex: 2, padding: '10px', background: '#c4a35a', border: 'none', borderRadius: '8px', color: '#1a1a1a', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>Edit Load</button>
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px' }}>
      {days.map(day => {
        ...
        return (
          <div key={dateStr} style={{ background: today ? 'rgba(196,163,90,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${today ? 'rgba(196,163,90,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '10px', padding: '10px', minHeight: '200px' }}>
            <div style={{ marginBottom: '8px' }}>
              <div style={{ color: today ? '#c4a35a' : '#94a3b8', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase' }}>{day.toLocaleDateString('en-US', { weekday: 'short' })}</div>
              <div style={{ color: today ? '#c4a35a' : '#f1f5f9', fontSize: '18px', fontWeight: '800', lineHeight: 1 }}>{day.getDate()}</div>
              {totalBarrels > 0 && <div style={{ color: '#475569', fontSize: '10px' }}>{totalBarrels.toLocaleString()} bbls</div>}
            </div>
            {dayLoads.map(load => <LoadCard key={load.id} load={load} onClick={onLoadClick} compact />)}
            <button onClick={() => onAddLoad(dateStr)} style={{ width: '100%', padding: '6px', background: 'transparent', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '6px', color: '#475569', fontSize: '11px', cursor: 'pointer', marginTop: '4px', fontFamily: 'inherit', transition: 'all 0.15s' }}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
        {days.map((day, i) => {
          ...
          const today = isToday(day);
          return (
            <div key={dateStr} style={{ background: today ? 'rgba(196,163,90,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${today ? 'rgba(196,163,90,0.3)' : 'rgba(255,255,255,0.06)'}`, borderRadius: '8px', padding: '6px', minHeight: '90px' }}>
              <div style={{ color: today ? '#c4a35a' : '#94a3b8', fontSize: '12px', fontWeight: '700', marginBottom: '4px' }}>{day.getDate()}</div>
              {dayLoads.slice(0, 2).map(load => <LoadCard key={load.id} load={load} onClick={onLoadClick} compact />)}
              {dayLoads.length > 2 && <div style={{ color: '#475569', fontSize: '10px', padding: '2px 4px' }}>+{dayLoads.length - 2} more</div>}
              <button onClick={() => onAddLoad(dateStr)} style={{ width: '100%', padding: '2px', background: 'transparent', border: 'none', color: '#334155', fontSize: '14px', cursor: 'pointer', lineHeight: 1 }}
    ...
    <div style={{ minHeight: '100vh', background: '#0a0f1a', color: '#f1f5f9', fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ display: 'flex', gap: '8px', color: '#64748b', fontSize: '13px' }}>
              ...
              {incompleteLoads > 0 && <><span>•</span><span style={{ color: '#f59e0b' }}>⚠ {incompleteLoads} incomplete</span></>}
            </div>
            <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: '8px', padding: '3px' }}>
              {['week', 'month'].map(mode => (
                <button key={mode} onClick={() => setViewMode(mode)} style={{ padding: '6px 16px', borderRadius: '6px', border: 'none', background: viewMode === mode ? '#c4a35a' : 'transparent', color: viewMode === mode ? '#1a1a1a' : '#94a3b8', fontWeight: viewMode === mode ? '700' : '500', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>{mode}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <WeekView loads={loads} weekStart={weekStart} onLoadClick={setSelectedLoad} onAddLoad={d => { setNewLoadDate(d); setShowNewLoad(true); }} />
        ) : (
          <MonthView loads={loads} currentDate={currentDate} onLoadClick={setSelectedLoad} onAddLoad={d => { setNewLoadDate(d); setShowNewLoad(true); }} />
        )}
      </div>

      {showNewLoad && (
        <NewLoadModal customers={customers} carriers={carriers} defaultDate={newLoadDate} userId={user?.id} onClose={() => { setShowNewLoad(false); setNewLoadDate(null); }} onSave={() => { setShowNewLoad(false); setNewLoadDate(null); fetchLoads(); }} />
      )}
